    ...
    with gzip.open('./tweets/conversations.jsonl.gz') as file:    
        for line in file:
            ...
            if len(processed_convos)%100000 == 0: 
                print(f'Execution after {len(processed_convos)} rows: {round(time.time() - start_time, 3)}s')
                execute_values(cur, "INSERT INTO conversation_references (conversation_id, parent_id, type) VALUES %s ON CONFLICT DO NOTHING;", references, page_size=100000)
                references = []

        if len(references) > 0:
            execute_values(cur, "INSERT INTO conversation_references (id, conversation_id, parent_id, type) VALUES %s ON CONFLICT DO NOTHING;", references, page_size=100000)
